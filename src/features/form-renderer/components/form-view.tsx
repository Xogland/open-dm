...
export default function FormView({
  ...
  const handleSubmit = async (submissionData: Record<string, any>) => {
    ...
    try {
      // Extract email and stripe payment id from workflow answers
      let email = "";
      ...
      const paymentEntry = values.find(
        (val) =>
          val &&
          typeof val === "object" &&
          val.type === "payment",
  ...
  return (
    ...
    >
      <div className="flex-1 flex items-center justify-center w-full min-h-0 overflow-hidden relative">
        <ChatFormView
          formData={{
