...
export default function ChatFormView({
  ...
  const addNextWorkflowStep = async (service: string, stepIndex: number) => {
    ...
    switch (type) {
      ...
      case "payment":
        const paymentStep = step as PaymentStep;
        const paymentMsg: PaymentInputMessage = {
          id: `payment_${Date.now()}`,
          type: "payment_input",
          question: paymentStep.question,
          stepId: paymentStep.id,
          amount: paymentStep.amount,
          currency: paymentStep.currency,
          description: paymentStep.description,
          publishableKey: organization?.stripeConfig?.publishableKey || "",
          timestamp: Date.now(),
        };
        stepMessage = paymentMsg;
  ...
  const organization = useQuery(api.organisation.getOrganisationByHandle, {
    handle: orgHandle as string,
  });
  ...
  const handleFileUpload = async (file: File) => {
    ...
    // Add a temporary system message to show upload progress
    ...
    try {
      ...
      // Remove temp message and add user response
  ...
  const handleFormSubmit = async (terminalStep?: WorkflowStep) => {
    ...
    try {
      ...
      const currentWorkflowStep = terminalStep || currentWorkflow[currentStepIndex];
      ...
      if (
        ...
        const extBrowserStep = currentWorkflowStep as ExternalBrowserStep;
        const redirectingMessage: ChatMessage = {
          id: "redirecting",
          type: "system_info",
          timestamp: Date.now(),
          text: "Redirecting...",
        };
        setMessages((prev) => [...prev, redirectingMessage]);

        // Format URL and redirect after a short delay
        const url = extBrowserStep.url;
      ...
      >
        <div className="h-full flex flex-col items-center justify-center p-8 text-center space-y-4">
          <div className="bg-black/5 p-4 rounded-full">
            <InboxIcon className="w-8 h-8 text-gray-900 opacity-50" />
          </div>
          <div className="space-y-2">
            <Typography variant="subheading" className="text-gray-900/80">
              Inbox at capacity
            </Typography>
            {/* <Typography variant="body" className="text-gray-900/40 text-sm max-w-[280px]">
