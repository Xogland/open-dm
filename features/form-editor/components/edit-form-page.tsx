...
import WorkflowRoutingEditor from "./workflow-routing-editor";
...
export default function EditFormPage() {
    ...
    const [activeTab, setActiveTab] = useState<"content" | "workflow" | "routing">("content");
    ...
    // Derive services list for the WorkflowEditor from Content
    const servicesList =
    ...
    return (
        <PageShell containerClassName="p-0 space-y-0" className="text-foreground">
            {/* Premium Header */}
            <div className="flex flex-col sm:flex-row w-full items-center justify-between p-4 md:px-8 bg-background/80 backdrop-blur-xl border-b border-border/50 z-10 shrink-0 gap-4">
                    ...
                    <Breadcrumb>
                        <BreadcrumbList>
                            <BreadcrumbItem>
                                <Link
                                    href="/dashboard"
                                    className="flex items-center gap-2 text-muted-foreground hover:text-primary transition-colors text-xs font-semibold uppercase tracking-wider"
                                >
                            ...
                            <BreadcrumbItem>
                                <BreadcrumbPage className="flex items-center gap-2 text-foreground text-xs font-bold uppercase tracking-wider">
                ...
                <div className="flex-1 flex justify-center">
                    <div className="flex bg-muted/40 rounded-xl p-1 shadow-sm border border-border/50">
                        <Button
                            ...
                            className={`rounded-lg text-xs font-bold px-6 h-8 transition-all duration-200 ${activeTab === "content" ? "bg-background text-primary shadow-sm" : "text-muted-foreground hover:bg-muted/60 hover:text-foreground"}`}
                        >
                            PROFILE & CONTENT
                        </Button>
                        ...
                        <Button
                            ...
                            className={`rounded-lg text-xs font-bold px-6 h-8 transition-all duration-200 ${activeTab === "workflow" ? "bg-background text-primary shadow-sm" : "text-muted-foreground hover:bg-muted/60 hover:text-foreground"}`}
                        >
                            CHAT FLOW
                        </Button>
                        ...
                        <Button
                            ...
                            className={`rounded-lg text-xs font-bold px-6 h-8 transition-all duration-200 ${activeTab === "routing" ? "bg-background text-primary shadow-sm" : "text-muted-foreground hover:bg-muted/60 hover:text-foreground"}`}
                        >
                            WORKFLOW
                        </Button>
                    </div>
                ...
                <div className="flex-none flex items-center gap-3">
                    {isViewer && (
                        <div className="hidden sm:flex items-center gap-2 px-3 py-1.5 bg-muted/50 rounded-lg border text-[10px] font-bold text-muted-foreground uppercase tracking-wider">
                            <Link2Icon className="h-3 w-3" />
                            View Only
                        </div>
                    )}
                    <Button variant="outline" size="sm" asChild className="h-10 px-4 rounded-xl border-border/60 hover:bg-muted shadow-sm transition-all font-semibold">
                    ...
                    <Button
                        ...
                        size="sm"
                        className="h-10 px-6 rounded-xl bg-primary shadow-lg shadow-primary/20 hover:shadow-primary/30 transition-all font-bold"
                    >
                        {isSaving ? (
            ...
            <div className="flex-1 w-full overflow-hidden">
                {activeTab === "content" ? (
                    ...
                    <WorkflowRoutingEditor
                        organisationId={selectedOrganization._id}
                        services={servicesList.map((s, idx) => ({ id: (data.content.services[idx]?.id || idx).toString(), title: s.title }))}
                        readOnly={isViewer}
                    />
                )}
            </div>
